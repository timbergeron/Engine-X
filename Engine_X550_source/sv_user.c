...
void SV_SetIdealPitch (void)
{
	...
	if (steps < 2)
		return;
	sv_player->v.idealpitch = -dir * sv_idealpitchscale.floater;
}
...
// if the leading edge is over a dropoff, increase friction
	...
	if (trace.fraction == 1.0)
		friction = sv_friction.floater * sv_edgefriction.floater;
	else
		friction = sv_friction.floater;

// apply friction
	control = speed < sv_stopspeed.floater ? sv_stopspeed.floater : speed;
...
void SV_Accelerate (void)
{
	...
	if (addspeed <= 0)
		return;
	accelspeed = sv_accelerate.floater*host_frametime*wishspeed;
...
void SV_AirAccelerate (vec3_t wishveloc)
{
	...
	if (addspeed <= 0)
		return;
//	accelspeed = sv_accelerate.floater * host_frametime;
	accelspeed = sv_accelerate.floater*wishspeed * host_frametime;
	if (accelspeed > addspeed)
		accelspeed = addspeed;
...
// user intentions
	...
	if (wishspeed > sv_maxspeed.floater)
	{
		VectorScale (wishvel, sv_maxspeed.floater/wishspeed, wishvel);
		wishspeed = sv_maxspeed.floater;
	}
	wishspeed *= 0.7;
...
// water friction
	...
	if (speed)
	{
		newspeed = speed - host_frametime * speed * sv_friction.floater;
		if (newspeed < 0)
			newspeed = 0;
...
void SV_NoclipMove (void)
{
	...
	if (VectorLength (velocity) > sv_maxspeed.floater)
	{
		VectorNormalize (velocity);
		VectorScale (velocity, sv_maxspeed.floater, velocity);
	}
}
...
void SV_ClientThink (void)
{
	...
	DropPunchAngle (); // Baker: does this fix JoeQuake RJ weirdness?
...
// show 1/3 the pitch angle and all the roll angle
	...
	angles[ROLL] = View_CalcRoll (sv_player->v.angles, sv_player->v.velocity) * 4;
...
// read current angles
	...
	if (0 /* BakerFebDebug !sv_fullpitch.integer */)
	{
...
#pragma message ("Quality assurance: Why are we not server pitch bounding?")
...
//				Sys_Printf ("clc_nop\n");
			...
			case clc_stringcmd:
				...
				if (!strncasecmp(s, "status", 6)	||
					...
					!strncasecmp(s, "spawn", 5)		||	// special
					!strncasecmp(s, "begin", 5)		||	// special
					!strncasecmp(s, "prespawn", 8)	||	// special
				...
				// nehahra specific
					!strncasecmp(s, "max", 3)		||
					!strncasecmp(s, "monster", 7)	||
					!strncasecmp(s, "scrag", 5)		||
					!strncasecmp(s, "gimme", 5)		||
					!strncasecmp(s, "wraith", 6)
					)
				{
					ret = 1;
				...
				{
					Con_DevPrintf (DEV_SERVER, "%s tried to %s\n", host_client->name, s);
				}
				break;
