...
qbool	serialAvailable = false;
qbool	ipxAvailable = false;
qbool	tcpipAvailable = false;
...
void (*GetComPortConfig)(int portNumber, int *port, int *irq, int *baud, qbool *useModem);
void (*SetComPortConfig)(int portNumber, int port, int irq, int baud, qbool useModem);
...
static char		rcon_buff[RCON_BUFF_SIZE];
sizebuf_t	rcon_message = {false, false, rcon_buff, RCON_BUFF_SIZE, 0};
qbool	rcon_active = false;

static qbool	configRestored = false;

FILE	*vcrFile = NULL;
qbool recording = false;	// OSX peeks at this in Quake.m
...
#ifndef _WIN32 // Windows specific rotate bits functions _lrotl and _lrotr.  Change to sys_lrotl, etc.
...
static void MaxPlayers_f (void)
{
	...
	Con_Printf ("Server: Deathmatch has been set to %i\n", pr_deathmatch.integer);
}
...
qsocket_t *NET_Connect (char *host)
{
	...
	if (host)
	{
		if (COM_StringMatchCaseless (host, "local"))
		{
			numdrivers = 1;
			goto JustDoIt;
		}
...
int NET_GetMessage (qsocket_t *sock)
{
	...
	{
		if (net_time - sock->lastMessageTime > net_messagetimeout.floater)
		{
			NET_Close (sock);
			return -1;
		}
		...
		if (net_time - sock->lastMessageTime > net_connecttimeout.floater && sv.active &&
			host_client && sock == host_client->netconnection && COM_StringMatch (host_client->name, "unconnected"))
		{
			NET_Close (sock);
			return -1;
		}
...
struct
{
	...
	long	session;
	int		r;
} static vcrSendMessage;

// JPG 3.20 - cheat free
static byte buff[NET_MAXMESSAGE];
static sizebuf_t newdata;
...
==================
*/
qbool NET_CanSendMessage (qsocket_t *sock)
{
	int	r;
...
void NET_Init (void)
{
	...
	{
		s = (qsocket_t *)Hunk_AllocName(1, sizeof(qsocket_t), "qsocket");
	...
	Cvar_Registration_Host_Network ();
	...
	if (*my_ipx_address)
		Con_DevPrintf (DEV_IO, "IPX address %s\n", my_ipx_address);
	if (*my_tcpip_address)
		Con_DevPrintf (DEV_IO, "TCP/IP address %s\n", my_tcpip_address);
...
{
	PollProcedure	*pp;
#ifdef SUPPORTS_MODEMS // Baker: Strip modem stuffs?
	qbool	useModem;
#endif
	...
	if (!configRestored)
	{
		...
		if (serialAvailable)
		{
			if (config_com_modem.floater != 0) // Baker 3.99: changed from == 1.0 to != 0
				useModem = true;
			else
				useModem = false;
			SetComPortConfig (0, (int)config_com_port.floater, (int)config_com_irq.floater, (int)config_com_baud.floater, useModem);
...
qbool IsID (struct qsockaddr *addr)
{
	if (idgods.floater == 0.0)
		return false;
