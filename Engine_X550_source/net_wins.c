...
void WINS_GetLocalAddress()
{
	...
	if (COM_CheckParm("-oldnet") || isDedicated)
	{
		...
		if (WINS_iptype(addr) == 1)
		{
			...
			Con_DevPrintf (DEV_IO, "WINS_GetLocalAddress: ip address #0 (%s) is invalid, checking next ...\n", my_tcpip_address);
			...
			if (local->h_addr_list[1])
			{
				...
				addr = ntohl (myAddr);
				snprintf (my_tcpip_address, sizeof(my_tcpip_address), "%d.%d.%d.%d", (addr >> 24) & 0xff, (addr >> 16) & 0xff, (addr >> 8) & 0xff, addr & 0xff);
				Con_DevPrintf (DEV_IO, "WINS_GetLocalAddress: ip address #1 (%s) found.  Using.\n", my_tcpip_address);
			}
			else
			{
				Con_DevPrintf (DEV_IO, "WINS_GetLocalAddress: ip address #1 does not exist\n");
				Con_DevPrintf (DEV_IO, "WINS_GetLocalAddress: Warning continuing with #0\n");
...
// so we can run on Win 3.1, where there isn't necessarily Winsock)
	...
	if (pgethostname(buff, MAXHOSTNAMELEN) == SOCKET_ERROR)
	{
		Con_DevPrintf (DEV_IO, "Winsock TCP/IP Initialization failed.\n");
		if (--winsock_initialized == 0)
			pWSACleanup ();
	...
	if (COM_StringMatch (sv_hostname.string, "UNNAMED"))
	{
		...
		Cvar_SetDefaultStringByRef (&sv_hostname, buff); // So that reset all works as I intend ... reset all should have all cvars reset to starting engine state.
	}
...
void PrintDNS_f (void) // (const char *host_addr)
{
   ...
   const char *host_addr = Cmd_Argv(1);
   struct in_addr addr = { 0 };

	if (Cmd_Argc() != 2) // More than just the command
	{
		Con_Printf ("Usage: %s <ip address>\n", Cmd_Argv (0));
		return;
	}
