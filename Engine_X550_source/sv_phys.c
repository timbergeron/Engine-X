...
// bound velocity
	for (i=0 ; i<3 ; i++)
	{
		...
		if (ent->v.velocity[i] > sv_maxvelocity.floater)
			ent->v.velocity[i] = sv_maxvelocity.floater;
		else if (ent->v.velocity[i] < -sv_maxvelocity.floater)
			ent->v.velocity[i] = -sv_maxvelocity.floater;
	}
}
...
void SV_AddGravity (edict_t *ent)
{
	...
	if (val && val->_float)
		...
		ent_gravity = 1.0;
	ent->v.velocity[2] -= ent_gravity * sv_gravity.floater * host_frametime;
}
...
void SV_PushMove (edict_t *pusher, float movetime)
{
	...
	int			mark; //johnfitz
#else
	edict_t		*moved_edict[DEFAULT_MAX_EDICTS];
	vec3_t		moved_from[DEFAULT_MAX_EDICTS];
#endif
...
#if SUPPORTS_BRUSHMODEL_ROTATION
/*
============
SV_PushRotate
...
void SV_CheckStuck (edict_t *ent)
{
	...
	if (!SV_TestEntityPosition(ent))
	{
		//Con_DevPrintf (DEV_PROTOCOL, "Unstuck.\n");  // JPG - commented this out
		SV_LinkEdict (ent, true);
		return;
	}
	...
	//Con_DevPrintf (DEV_PROTOCOL, "player is stuck.\n");    // JPG - commented this out
}
...
=============
*/
qbool SV_CheckWater (edict_t *ent)
{
	vec3_t	point;
	int		cont;
...
// decide which move function to call
	...
	case MOVETYPE_WALK:
		...
		if (!SV_CheckWater(ent) && !((int)ent->v.flags & FL_WATERJUMP))
			SV_AddGravity (ent);		// Add gravity to velocity, if not in water
		SV_CheckStuck (ent);
		SV_WalkMove (ent);
	...
	case MOVETYPE_NOCLIP:
		if (!SV_RunThink(ent))
			return;
		VectorMultiplyAdd (ent->v.origin, host_frametime, ent->v.velocity, ent->v.origin);
		break;
...
// regular thinking
	...
	VectorMultiplyAdd (ent->v.angles, host_frametime, ent->v.avelocity, ent->v.angles);
	VectorMultiplyAdd (ent->v.origin, host_frametime, ent->v.velocity, ent->v.origin);
...
#define GRENADE_BOUNCE_FIX
	...
	if (trace.plane.normal[2] > 0.7)
	{
		...
		if (sv_bouncedownslopes.integer)
		{
...
// freefall if not onground
	...
	{
		// Not on ground ... fall ....
		if (ent->v.velocity[2] < sv_gravity.floater*-0.1)
			hitsound = true;
		else
			hitsound = false;

		SV_AddGravity (ent);			// Add gravity to velocity
		SV_CheckVelocity (ent);			// Caps to sv.maxvelocity also checks for NaN errors
		SV_FlyMove (ent, host_frametime, NULL);
		SV_LinkEdict (ent, true);
...
//SV_CheckAllEnts ();		// Baker: commented out in original Q1 source
...
trace_t SV_Trace_Toss (edict_t *ent, edict_t *ignore)
{
	...
	while (1)
	{
		...
		SV_AddGravity (tent);
		VectorMultiplyAdd (tent->v.angles, host_frametime, tent->v.avelocity, tent->v.angles);
