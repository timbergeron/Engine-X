...
// make a stack frame
	...
	while (1)
	{
		...
		case OP_NOT_V:
			...
			break;
		case OP_NOT_S:
				c->_float = !a->string || !pr_strings[a->string];	// Baker: dereferenced
			break;
		case OP_NOT_FNC:
			c->_float = !a->function;
			break;
		...
		case OP_EQ_V:
			...
			break;
		case OP_EQ_S:
		//	c->_float = COM_StringMatch (pr_strings+a->string,pr_strings+b->string);
			c->_float = COM_StringMatch (PR_GETSTRING(a->string), PR_GETSTRING(b->string));
			break;
		case OP_EQ_E:
