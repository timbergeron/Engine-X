...
char *PF_VarString (int	first)
{
	...
	{
		StringLCopy (out, G_STRING((OFS_PARM0+i*3)));
	}
	return out;
}
...
// check to see if model was properly precached
	for (i=0, check = sv.model_precache ; *check ; i++, check++)
		if (COM_StringMatch (*check, m))
			break;

//	DMSG(va("pf_setmodel %s", m));

	if (!*check)
		PR_RunError ("no precache: %s\n", m);

	e->v.model = PR_SETSTRING(m); //  - pr_strings;
...
void PF_cvar (void)
{
	...
	G_FLOAT(OFS_RETURN) = Cvar_GetFloatByName (str);
}
...
void PF_cvar_set (void)
{
	...
	if (!(var = Cvar_FindVarByName(var_name)))
	{
...
void PF_dprint (void)
{
	Con_DevPrintf (DEV_PROTOCOL, "%s",PF_VarString(0));
}
...
void PF_ftos (void)
{
	...
	G_INT(OFS_RETURN) = PR_SETTMPSTRING(pr_string_temp);
}
...
void PF_precache_sound (void)
{
	...
	for (i=0 ; i<MAX_SOUNDS ; i++)
	{
		...
		if (COM_StringMatch (sv.sound_precache[i], s))
			return;
	}
...
void PF_precache_model (void)
{
	...
	for (i=0 ; i<MAX_MODELS ; i++)
	{
		...
		if (COM_StringMatch (sv.model_precache[i], s))
			return;
	}
...
// try sending a trace straight
	...
	if (tr.ent && tr.ent->v.takedamage == DAMAGE_AIM && (!pr_teamplay.floater || ent->v.team <= 0 || ent->v.team != tr.ent->v.team))
	{
...
// try all possible entities
	...
	for (i=1 ; i<sv.num_edicts ; i++, check = NEXT_EDICT(check))
	{
		...
		if (check == ent)
			continue;
		if (pr_teamplay.floater && ent->v.team > 0 && ent->v.team == check->v.team)
