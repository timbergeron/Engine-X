...
int	Sys_FileTime (const char *thePath)
{
...
//    NSRunAlertPanel(nil, msg, nil, nil, nil);
	...
	if (!session_confirmquit.integer)
		return 1; // They always want to quit
    
    int iResponse = NSRunAlertPanel(@"Confirm Exit", @"Are you sure you want to quit?", @"Yes", @"No", nil);

    switch(iResponse) {
        case NSAlertDefaultReturn:    /* user pressed Yes */
          return 1;
        case NSAlertAlternateReturn:  /* user pressed No */
	default:
          break;
    }
    return 0;
...
void	Sys_Quit (void)
{
    ...
    NSString		*myString;
    SInt			mySamples = gl_fsaa.floater;
...
{
	// Baker: Do nothing!  Isn't Windows ;)
}
...
void *	Sys_GetProcAddress (const char *theName /*, Boolean theSafeMode*/)
{
    ...
    if (/*theSafeMode == YES .. Baker is always NO for now  && */ mySymbol == NULL)
    {
...
int	Sys_CheckSpecialKeys (int theKey)
{
    ...
    if (keydown[K_LWIN /*K_COMMAND*/] == false)
    {
        return (0);
    }
    ...
    switch (myKey)
    {
        ...
        case 'M':
            ...
            if (gVidDisplayFullscreen == NO && Minimized == NO && gVidWindow != NULL)
            {
    ...
    switch (myType)
    {
            ...
            if ([myEvent subtype] == 7)
            {
                ...
                for (i = 0; i < IN_MOUSE_BUTTONS; i++)
                {
                    if(myFilteredMouseButtons & (1 << i))
                        Key_Event (K_MOUSE1 + i, 0, (myMouseButtons & (1 << i)) ? 1 : 0);
                }
        ...
        case NSKeyUp:
            ...
            for (i = 0; i < myKeyboardBufferSize; i++)
            {
                ...
                if ((myCharacter & 0xFF00) ==  0xF700)
                {
                    ...
                    if (myCharacter < 0x47)
                    {
                        Key_Event (gInSpecialKey[myCharacter], 0, (myType == NSKeyDown));
                    }
                }
                else
                {
                    ...
                    if (myFlags & NSNumericPadKeyMask)
                    {
                        ...
                        {
                            Key_Event (gInNumPadKey[myKeyPad], 0, (myType == NSKeyDown));
                            break;
                        }
                    }
                    if (myCharacter < 0x80)
                    {
                        if (myCharacter >= 'A' && myCharacter <= 'Z')
                            myCharacter += 'a' - 'A';
                        Key_Event (myCharacter, 0, (myType == NSKeyDown));
                    }
                }
            }
        ...
        case NSFlagsChanged:
            ...
            if (myFilteredFlags & NSAlphaShiftKeyMask)
                Key_Event (K_CAPSLOCK, 0, (myFlags & NSAlphaShiftKeyMask) ? 1:0);
                
            if (myFilteredFlags & NSShiftKeyMask)
                Key_Event (K_SHIFT, 0, (myFlags & NSShiftKeyMask) ? 1:0);
                
            if (myFilteredFlags & NSControlKeyMask)
                Key_Event (K_CTRL, 0, (myFlags & NSControlKeyMask) ? 1:0);
                
            if (myFilteredFlags & NSAlternateKeyMask)
                Key_Event (K_ALT, 0, (myFlags & NSAlternateKeyMask) ? 1:0);
                
            if (myFilteredFlags & NSCommandKeyMask)
                Key_Event (K_LWIN /*K_COMMAND*/, 0, (myFlags & NSCommandKeyMask) ? 1:0);
                
            if (myFilteredFlags & NSNumericPadKeyMask)
                Key_Event (KP_NUMLOCK, 0, (myFlags & NSNumericPadKeyMask) ? 1:0);
...
{
	char	*myData = buffer_to_modify;
	// Baker: this replaces carriage return characters with linefeeds
    
    while (*myData != 0x00)		// Return to null terminator
	{
		if (*myData == 0x0D)	// Find carriage returns	"\r"
			*myData = 0x0A;		// Change to line feeds		"\n"
...
SInt	main (SInt theArgCount, const char **theArgValues)
{
	...
	if (sys_extrasleep.floater)
            usleep (sys_extrasleep.floater);
    }	
