...
void	VID_SetGamma (void)
{
    ...
    if (myOldGamma != vid_brightness_gamma.floater || myOldOverbright != vid_overbright.floater)
    {
        if (vid_overbright.floater == 0.0f)
        {
            if (gVshGammaTable != NULL)
            {
                ...
                myOldGamma = (1.4f - vid_brightness_gamma.floater) * 2.5f;
                if (myOldGamma < 1.0f)
                {
                    myOldGamma = 1.0f;
                }
        ...
        else
        {
            if (gVshOriginalGamma != NULL)
            {
                ...
                myOldGamma = 1.0f - vid_brightness_gamma.floater;
                if (myOldGamma < 0.0f)
                {
                    myOldGamma = 0.0f;
                }
            ...
            // has to be 0 or 1:
            if (vid_overbright.floater != 1.0f)
            {
                Cvar_SetFloatByRef (&vid_overbright, 1.0f);
            }
        }
        myOldGamma = vid_brightness_gamma.floater;
        myOldOverbright = vid_overbright.floater;
    }
}
const qbool movie_is_capturing = false;
...
void	VID_SetPaletteOld (UInt8 *thePalette)
{
...
void	VID_ShiftPaletteOld (UInt8 *thePalette)
{
...
SInt 	VID_SetMode (SInt modenum, qbool IsReset/* UInt8 *thePalette*/)
{
    return (1);
}
...
void VID_SetWait (UInt32 theState)
{
    ...
    {
        gGLVideoWait = vid_vsync.integer;
        if (theState == 0)
        {
    ...
    {
        vid_vsync.integer = gGLVideoWait;
...
#else
#define ENGINE_NAME_OSX "Engine X 5.50 Beta" // Must be manually updated for now
        [gVidWindow setTitle: @ENGINE_NAME_OSX];
#endif /* QUAKE_WORLD */
    ...
    VID_SetWait((UInt32) vid_vsync.integer);
...
void	VID_Init (void /*unsigned char *thePalette*/)
{
...
// EngineX'd    vid.maxwarpwidth = VID_WARP_WIDTH;
//EngineX'd    vid.maxwarpheight = VID_WARP_HEIGHT;
...
//    Sys_mkdir (myGLDir);
 ...
 // BakerDanger   VID_CheckGamma (thePalette);
 // BakerDanger    VID_SetPaletteOld (thePalette);
...
void	VID_MenuDraw (void)
{
    ...
    if (vid_vsync.integer) M_Print ((39 - 2) * VID_FONT_WIDTH, myRow, "On");
    ...
    if (vid_overbright.floater) M_Print ((39 - 2) * VID_FONT_WIDTH, myRow, "On");
        else M_Print ((39 - 3) * VID_FONT_WIDTH, myRow, "Off");
    ...
    if (gl_fsaaavailable == YES)
    {
        ...
        if (gl_fsaa.floater == 0.0f)
        {
             M_Print ((39 - 3) * VID_FONT_WIDTH, myRow, "Off");
        }
        else
        {
            snprintf (myString, 16, "%dx", (int) gl_fsaa.floater);
...
*/
    ...
    if (gl_pntriangles == YES)
    {
        ...
        if (gl_truform.floater < 0)
        {
             M_Print ((39 - 3) * VID_FONT_WIDTH, myRow, "Off");
        }
        else
        {
            snprintf (myString, 16, "%dx", (int) gl_truform.floater);
            M_Print ((39 - strlen (myString)) * VID_FONT_WIDTH, myRow, myString);
        }
...
void	VID_MenuKey (int theKey)
{
        ...
        case K_LEFTARROW:
            ...
            switch (gGLMenuItem)
            {
                case VID_MENUITEM_WAIT:
                    Cvar_SetFloatByRef (&vid_vsync, (vid_vsync.integer == 0.0f) ? 1.0f : 0.0f);
                    break;
                case VID_MENUITEM_OVERBRIGHT:
                    Cvar_SetFloatByRef (&vid_overbright, (vid_overbright.floater == 0.0f) ? 1.0f : 0.0f);
                    break;
                case VID_MENUITEM_FSAA:
                    Cvar_SetFloatByRef (&gl_fsaa, (gl_fsaa.floater <= 0.0f) ? 8.0f : gl_fsaa.floater - 4.0f);
                    break;
                case VID_MENUITEM_ANISOTROPIC:
                    Cvar_SetFloatByRef (&gl_anisotropic, (gl_anisotropic.floater == 0.0f) ? 1.0f : 0.0f);
                    break;
    /*
	            case VID_MENUITEM_MULTITEXTURE:
                    Cvar_SetFloatByRef (&gl_multitexture, (gl_multitexture.floater == 0.0f) ? 1.0f : 0.0f);
                    break;
    */
				case VID_MENUITEM_TRUFORM:
                    Cvar_SetFloatByRef (&gl_truform, (gl_truform.floater <= -1.0f) ? 7.0f : gl_truform.floater-1.0f);
                    break;
            }
            break;
        case K_RIGHTARROW:
		case K_ENTER:
            ...
            switch (gGLMenuItem)
            {
                ...
                case VID_MENUITEM_OVERBRIGHT:
                    ...
                    break;
                case VID_MENUITEM_FSAA:
                    Cvar_SetFloatByRef (&gl_fsaa, (gl_fsaa.floater >= 8.0f) ? 0.0f : gl_fsaa.floater + 4.0f);
                    break;
                case VID_MENUITEM_ANISOTROPIC:
                    Cvar_SetFloatByRef (&gl_anisotropic, (gl_anisotropic.floater == 0.0f) ? 1.0f : 0.0f);
                    break;
	...
	*/
                case VID_MENUITEM_TRUFORM:
                    Cvar_SetFloatByRef (&gl_truform, (gl_truform.floater >= 7.0f) ? -1.0f : gl_truform.floater+1.0f);
                    break;
            }
            break;
        default:
...
void	GL_CheckSwitchFSAAOnTheFly (void)
{
    ...
    Cvar_SetFloatByRef (&gl_fsaa, gGLFSAALevel);
}
...
void	GL_OSX_Init (void)
{
//#ifdef SUPPORTS_HLBSP
//	void BuildGammaTable (float g);
//	Build_Gamma_Table();   // Baker: occurs in cl_view.c in init anyway so wtf did I put this here?
//#endif
	GL_Init ();
...
void	GL_SetPNTriangles (SInt32 thePNTriangleLevel)
{
    ...
    if (gl_pntriangles == NO)
    {
		...
		gGLPNTriangleLevel = (gl_truform.floater > 7.0f) ? 7.0f : gl_truform.floater;
        ...
        Cvar_SetFloatByRef (&gl_truform, gGLPNTriangleLevel);
        return;
    }
...
void	GL_SetTextureFilterAnisotropic (UInt32 theState)
{
    ...
    Cvar_SetFloatByRef (&gl_anisotropic, gGLAnisotropic);
...
void	GL_SetMultiTexture (UInt32 theState)
{
    ...
    Cvar_SetFloatByRef (&gl_multitexture, gGLMultiTexture);
...
void	GL_EndRendering (void)
{
    ...
    if(vid_vsync.integer != gGLVideoWait)
    {
        VID_SetWait ((UInt32) vid_vsync.integer);
    }

    // check if anisotropic texture filtering changed:
    if (gl_anisotropic.floater != gGLAnisotropic)
    {
        GL_SetTextureFilterAnisotropic ((UInt32) gl_anisotropic.floater);
    }

    // check if vid_fsaa changed:
    if (gl_fsaa.floater != gGLFSAALevel)
    {
        GL_SetFSAA ((UInt32) gl_fsaa.floater);
    }

    // check if truform changed:
    if (gl_truform.floater != gGLPNTriangleLevel)
    {
        GL_SetPNTriangles ((SInt32) gl_truform.floater);
    }
/*
    // check if multitexture changed:
    if (gl_multitexture.floater != gGLMultiTexture)
    {
        GL_SetMultiTexture ((UInt32) gl_multitexture.floater);
    }
*/
}
