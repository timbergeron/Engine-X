...
void ResampleSfx (sfx_t *sfx, const int inrate, const int inwidth, byte *data)
{
	...
	do
	{
		...
		if (snd_loadas8bit.integer)	sc->width			= 1;
...
//	Con_Printf ("loading %s\n",namebuffer);
	...
	if (!(data = QFS_LoadStackFile(namebuffer, stackbuf, sizeof(stackbuf), NULL)))
	{
#if 0
		if (mod_nosoundwarn) // Developer print it instead if -nosoundwarn used
			Con_DevPrintf (DEV_PROTOCOL, "Couldn't load %s\n", namebuffer);
		else
#endif
	...
	info = GetWavinfo (s->name, data, qfs_lastload.filelen);
	if (info.channels != 1)
	{
