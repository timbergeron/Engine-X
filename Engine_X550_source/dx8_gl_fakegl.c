...
// Baker:
#define DX8_USE_POLYGON_OFFSET 0 // I cannot get this to work very well.
// End Baker
...
#include "dxsdk/sdk8/include/d3d8.h"
#include "dxsdk/sdk8/include/d3dx8.h"

// link to d3d8 libraries this way do that we don't need to modify the project
#pragma comment (lib, "dxsdk/sdk8/lib/d3d8.lib")
#pragma comment (lib, "dxsdk/sdk8/lib/d3dx8.lib")
...
{
	extern	int	d3d_vsync_state;	// Baker: Evile
	...
	if (windowed_)
	{
		...
		d3d_vsync_state = false;

	}
	else
	{
		// Baker: Vile evil
		float	Cvar_GetFloatByName (const char *var_name);	// Obtain if vsync wanted
		...
		if (Cvar_GetFloatByName("vid_vsync") == 1.0f)
		{
			//Con_Success ("Direct3D vertical sync requested\n");
			d3d_PresentParams.FullScreen_PresentationInterval = D3DPRESENT_INTERVAL_DEFAULT;
			d3d_vsync_state = true;
		}
		else
		{
...
BOOL WINAPI d3dmh_wglMakeCurrent (HDC hdc, HGLRC hglrc)
{
	...
	{
		// Baker: We need to detect fullscreen windowed mode here.
		//        So we can tell it to create a window
		extern int gVileHack_IsFullScreenWindow;
		if (gVileHack_IsFullScreenWindow)
...
#if 0
/*
MH's void D3DVid_ResizeWindow (HWND hWnd)
{
	if (!d3d_Device) return;
...
#if 1
void D3D_ResizeWindow (void)  // d3d_PresentParams.hDeviceWindow
{
	RECT clientrect;
...
//	if (d3d_CurrentMode.RefreshRate != 0) return;
	...
	// reset the device to resize the backbuffer
	GetClientRect (d3d_PresentParams.hDeviceWindow, &clientrect);
...
//	vid.recalc_refdef = 1;		// No the shared code can do that
//	IN_UpdateClipCursor ();		// No the shared code can do that
//	D3DVid_Restart_f ();
}
#endif


void D3D_ResetWindow (RECT new_clientrec, const int newWidth, const int newHeight, const int WhateverBpp)
{
...
//		Sleep (1);
	...
	D3D_SetupPresentParams (newWidth, newHeight, WhateverBpp, true /*windowed_*/);
...
//	Con_Printf ("Resized Window\n");

}

void D3D_ResetMode (int width, int height, int bpp, int *Window_X, int *Window_Y, int windowed_) // Baker: Public
{
	RECT winrect;
	...
	switch (windowed_)
	{
	case 0: winstyle = WS_POPUP; break;
	case 1:
		{	extern int gWindowStandardStyle;  /*Was: WS_OVERLAPPED | WS_BORDER | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX;*/
			winstyle = gWindowStandardStyle;
			break;
		}

	case 2: winstyle = WS_POPUP; break;
	}
...
//	Con_Printf ("D3D_Reset is windowed is %i\n", windowed_);
	_window_x = windowed_ ? (d3d_DesktopMode.Width - (winrect.right - winrect.left)) / 2 : 0;
	_window_y = windowed_ ? (d3d_DesktopMode.Height - (winrect.bottom - winrect.top)) / 2 : 0,
	...
	(
		...
		winrect.bottom - winrect.top,
//		SWP_NOOWNERZORDER | SWP_NOREPOSITION | SWP_NOZORDER | SWP_SHOWWINDOW
//		SWP_NOOWNERZORDER | SWP_NOREPOSITION /* " SWP_NOREPOSITION Same as the SWP_NOOWNERZORDER flag" /*| SWP_NOZORDER | SWP_SHOWWINDOW

//SetWindowPos (mainwindow, HWND_TOP, 0, 0, 0, 0,         SWP_DRAWFRAME | SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW | SWP_NOCOPYBITS);
//SetWindowPos (hWndCenter, NULL, CenterX, CenterY, 0, 0, SWP_DRAWFRAME |              SWP_NOSIZE | SWP_SHOWWINDOW | SWP_NOZORDER);
		SWP_DRAWFRAME |              SWP_NOSIZE | SWP_SHOWWINDOW | SWP_NOZORDER
	);
...
PROC WINAPI d3dmh_wglGetProcAddress (LPCSTR s)
{
	...
	for (i = 0; ; i++)
	{
		...
		if (COM_StringMatch (s, d3d_EntryPoints[i].funcname))
		{
