...
static MPEGLAYER3WAVEFORMAT m_mp3_format;
static qbool	mp3_encoder_exists;
static HACMDRIVER	m_mp3_driver;
static HACMSTREAM	m_mp3_stream;
static ACMSTREAMHEADER	m_mp3_stream_header;

extern qbool avi_loaded, acm_loaded;
	...
	if (fdwSupport & ACMDRIVERDETAILS_SUPPORTF_CODEC)
	{
		...
		for (i = 0 ; i < drvDetails.cFormatTags ; i++)
		{
			...
			if (fmtDetails.dwFormatTag == WAVE_FORMAT_MPEGLAYER3)
			{
				...
				Con_DevPrintf (DEV_DEMOS, "MP3-capable ACM codec found: %s\n", drvDetails.szLongName);
				...
				if ((mmr = qacmStreamOpen(&m_mp3_stream, m_mp3_driver, &m_wave_format, &m_mp3_format.wfx, NULL, 0, 0, 0)))
				{
					...
					case MMSYSERR_INVALPARAM:
						Con_DevPrintf (DEV_DEMOS, "ERROR: Invalid parameters passed to acmStreamOpen\n");
						break;

					case ACMERR_NOTPOSSIBLE:
						Con_DevPrintf (DEV_DEMOS, "ERROR: No ACM filter found capable of encoding MP3\n");
						break;

					default:
						Con_DevPrintf (DEV_DEMOS, "ERROR: Couldn't open ACM encoding stream\n");
						break;
					}
					continue;
				}
...
qbool Capture_Open (char *filename)
{
	...
	m_audio_is_mp3 = (qbool)capture_mp3.floater;
	...
	stream_header.dwRate = (unsigned long)(0.5 + capture_fps.floater);
	...
	if (m_audio_is_mp3)
	{
		...
		m_mp3_format.wfx.nAvgBytesPerSec = capture_mp3_kbps.floater * 125;
		...
		m_mp3_format.nBlockSize = m_mp3_format.wfx.nAvgBytesPerSec / capture_fps.floater;
...
void Capture_WriteAudio (int samples, byte *sample_buffer)
{
	...
	if (m_audio_is_mp3)
	{
		...
		mp3_buffer = Q_malloc (mp3_bufsize, "AVI mp3 buffer");
