...
// decide which clipping hull to use, based on the size
	...
	{	// explicit hulls in the BSP model
		...
		if (!model || model->modelformat != mod_brush)
...
===============
*/
static void SV_FindTouchedLeafs (edict_t *ent, const mnode_t *node)
{
	mplane_t	*splitplane;
	mleaf_t		*leaf;
	int		sides, leafnum;
...
==================
*/
int SV_HullPointContents (const hull_t *hull, const int in_num, const vec3_t p)
{
	float		d;
	int			num = in_num;
#if FITZQUAKE_PROTOCOL
