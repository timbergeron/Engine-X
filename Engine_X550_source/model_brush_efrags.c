...
void Efrags_CheckEfrags (void)
{
	...
	if (count > MAX_WINQUAKE_EFRAGS && dev_peakstats.efrags <= MAX_WINQUAKE_EFRAGS)
		Con_Warning ("%i efrags exceeds standard limit of %i.\n", count, MAX_WINQUAKE_EFRAGS);
#pragma message ("Quality assurance: Activate efrag checking.  No reason to not do this.")
...
===================
*/
static void Efrags_AddEfrags_SplitEntityOnNode_Recursive (mnode_t *node)
{
	efrag_t		*ef;
	mplane_t	*splitplane;
	mleaf_t		*leaf;
...
// recurse down the contacted sides
	if (sides & 1)
		Efrags_AddEfrags_SplitEntityOnNode_Recursive (node->children[0]);

	if (sides & 2)
		Efrags_AddEfrags_SplitEntityOnNode_Recursive (node->children[1]);
}
...
void Efrags_AddEfrags (entity_t *ent)
{
	...
	Efrags_AddEfrags_SplitEntityOnNode_Recursive (cl.worldmodel->nodes);

	ent->topnode = r_pefragtopnode;

#if FITZQUAKE_PROTOCOL
	Efrags_CheckEfrags (); //johnfitz
#endif
}
...
void Efrags_StoreEfrags (efrag_t **ppefrag)
{
	...
	while ((pefrag = *ppefrag) != NULL)
	{
		...
		{
			cl_visedicts[cl_numvisedicts++] = pent; // Baker: This adds the entity into the visibility list
			pent->visframe = r_framecount;			// Baker
		}
