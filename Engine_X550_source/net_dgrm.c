...
//extern int	m_state;
extern qbool	m_return_onerror;
extern char	m_return_reason[32];


#ifdef _DEBUG // NET_DEBUG
...
#pragma message ("Quality assurance: I think the above 4 statements go outside the #IFDEF.  Check versus older source or something.")

	if (data->cursize > NET_NETQUAKE_NET_MAXMESSAGE)
		Con_Printf("Datagram_SendMessage: message size %u exceeds NETQUAKE maxsize message %u\n", data->cursize, NET_NETQUAKE_NET_MAXMESSAGE);

	if (data->cursize > MAX_WINQUAKE_DATAGRAM)
		Con_Printf("Datagram_SendMessage: message size %u exceeds NETQUAKE datagram maxsize message %u\n", data->cursize, MAX_WINQUAKE_DATAGRAM);

#pragma message ("Quality assurance: Fix this datagram and max message issue.  Regular NQ engines cannot connect to Engine X due to this.")
...
qbool Datagram_CanSendMessage (qsocket_t *sock)
{
	if (sock->sendNext)
...
qbool Datagram_CanSendUnreliableMessage (qsocket_t *sock)
{
	return true;
}
...
//		continue;
		...
		{
#ifdef _DEBUG // NET_DEBUG
			Con_DevPrintf (DEV_IO, "Forged packet received\n");
			Con_DevPrintf (DEV_IO, "Expected: %s\n", StrAddr(&sock->addr));
			Con_DevPrintf (DEV_IO, "Received: %s\n", StrAddr(&readaddr));
#endif
			continue;
		}
		...
		if (flags & NETFLAG_UNRELIABLE)
		{
			...
			{
				Con_DevPrintf (DEV_IO, "Got a stale datagram\n");
				ret = 0;
				break;
			}
			if (sequence != sock->unreliableReceiveSequence)
			{
				...
				Con_DevPrintf (DEV_IO, "Dropped %u datagram(s)\n", count);
			}
			...
			SZ_Write (&net_message, packetBuffer.data, length, false /* not command buffer*/);
		...
		if (flags & NETFLAG_ACK)
		{
			...
			{
				Con_DevPrintf (DEV_IO, "Stale ACK received\n");
				continue;
			}
			if (sequence == sock->ackSequence)
			{
				...
				if (sock->ackSequence != sock->sendSequence)
					Con_DevPrintf (DEV_IO, "ack sequencing error\n");
			}
			else
			{
				Con_DevPrintf (DEV_IO, "Duplicate ACK received\n");
				continue;
			}
		...
		if (flags & NETFLAG_DATA)
		{
			...
			{
				SZ_Clear (&net_message);
				SZ_Write (&net_message, sock->receiveMessage, sock->receiveMessageLength, false /* not command buffer*/);
				SZ_Write (&net_message, packetBuffer.data, length, false /* not command buffer*/);
...
void NET_Stats_f (void)
{
	...
	else if (COM_StringMatch (Cmd_Argv(1), "*"))
	{
	...
	{
		for (s = net_activeSockets ; s ; s = s->next)
			if (COM_StringMatchCaseless (Cmd_Argv(1), s->address))
				break;
		if (s == NULL)
			for (s = net_freeSockets ; s ; s = s->next)
				if (COM_StringMatchCaseless(Cmd_Argv(1), s->address))
...
static qbool testInProgress = false;
...
static void Test_f (void)
{
	...
	if (host && hostCacheCount)
	{
		for (n=0 ; n<hostCacheCount ; n++)
			if (COM_StringMatchCaseless (host, hostcache[n].name))
			{
...
static qbool test2InProgress = false;
...
#ifdef _WIN32 // Winsock tied
		Cmd_AddCommand ("printdns", PrintDNS_f);
#endif
	}
...
void Datagram_Listen (qbool state)
{
	int	i;
...
static qsocket_t *_Datagram_CheckNewConnections (void)
{
	...
	if (command == CCREQ_SERVER_INFO)
	{
		// JPG 3.50
		char name[256];
		StringLCopy (name, sv_hostname.string);
		if (pq_cheatfree)
			StringLCat (name, " (cheat-free)");

		if (COM_StringNOTMatch (MSG_ReadString(), "QUAKE"))
			return NULL;
		...
		MSG_WriteString(&net_message, name);	// JPG 3.50 changed sv_hostname.string to name
		MSG_WriteString (&net_message, sv.worldname);
	...
	if (command == CCREQ_RULE_INFO)
	{
		...
		if (*prevCvarName)
		{
			var = Cvar_FindVarByName (prevCvarName);
			if (!var)
				return NULL;
			var = var->next;
		}
		...
		if (var)
		{
			MSG_WriteString (&net_message, var->cvarname);
...
//#ifdef RCON_PROTECTION
		else if (COM_StringNOTMatch (pass, rcon_password.string))
...
//#endif
	...
	if (mod != MOD_QSMACK)
	{
		if (pq_password.floater && (len <= 18 || pq_password.floater != MSG_ReadLong()))
...
static void _Datagram_SearchForHosts (qbool xmit)
{
	int		i, n, ret, control;
	struct qsockaddr readaddr;
	...
	while ((ret = dfunc.Read(dfunc.controlSock, net_message.data, net_message.maxsize, &readaddr)) > 0)
	{
		...
		if (MSG_ReadByte() != NET_PROTOCOL_VERSION)
		{
			...
			StringLCat (hostcache[n].name, hostcache[n].cname);
		}
		...
		for (i=0 ; i<hostCacheCount ; i++)
		{
			if (i == n)
				continue;
			if (COM_StringMatchCaseless (hostcache[n].name, hostcache[i].name))
			{
...
void Datagram_SearchForHosts (qbool xmit)
{
	for (net_landriverlevel = 0 ; net_landriverlevel < net_numlandrivers ; net_landriverlevel++)
	{
		if (hostCacheCount == HOSTCACHESIZE)
...
static qsocket_t *_Datagram_Connect (char *host)
{
	...
	for (reps=0 ; reps<3 ; reps++)
	{
		...
		MSG_WriteLong(&net_message, pq_password.floater);		// JPG 3.00 - password protected servers
		*((int *)net_message.data) = BigLong (NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
		...
		do
		{
			ret = dfunc.Read (newsock, net_message.data, net_message.maxsize, &readaddr);
                        ...
                        if (sfunc.AddrCompare(&readaddr, &sendaddr) != 0)
                        {
                            ...
                            if (COM_StringMatchCaseless (mySendName, myReadName))
                            {
