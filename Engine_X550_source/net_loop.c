...
static qbool	localconnectpending = false;
...
void Loop_SearchForHosts (qbool xmit)
{
	...
	hostCacheCount = 1;
	if (strcmp(sv_hostname.string, "UNNAMED") == 0)
		strcpy (hostcache[0].name, "local");
	else
		strcpy (hostcache[0].name, sv_hostname.string);
	strcpy (hostcache[0].map, sv.worldname);
...
{
	if (COM_StringNOTMatch (host,"local"))
		return NULL;
...
int Loop_GetMessage (qsocket_t *sock)
{
	...
	SZ_Write (&net_message, &sock->receiveMessage[4], length, false /* not command buffer*/);
