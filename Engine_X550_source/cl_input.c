...
//
// ProQuake:
// Engine X: CL_BaseMove difference ... consider implications, pq_lag differences
// ***************
...
void KeyDown (kbutton_t *b)
{
	...
	if (b == &in_jump && in_smartjump.integer && cl.stats[STAT_HEALTH] > 0 && cl.inwater)
		b = &in_up;

	/* FIXME - remove this if the other code works
	if (b == &in_jump && in_smartjump.integer && cl.stats[STAT_HEALTH] > 0)
	{
		int	cont;
...
void KeyUp (kbutton_t *b)
{
	...
	if (b == &in_jump && in_smartjump.integer)
	{
...
{
	KeyUp(&in_mlook);
	if (!mlook_active && in_lookspring.integer)
...
void CL_AdjustAngles (void)
{
	...
	speed = (in_speed.state & 1) ? host_frametime * keyboard_speed_angle_multiplier.floater : host_frametime;

#if SUPPORTS_XFLIP
	if (scene_invert_x.integer) cl.viewangles[YAW] *= -1;   //Atomizer - GL_XFLIP
#endif

	if (!(in_strafe.state & 1))
	{
		cl.viewangles[YAW] -= speed * keyboard_speed_yaw.floater * CL_KeyState (&in_right);
		cl.viewangles[YAW] += speed * keyboard_speed_yaw.floater * CL_KeyState (&in_left);
	...
	{
		View_StopPitchDrift ();
		cl.viewangles[PITCH] -= speed * keyboard_speed_pitch.floater * CL_KeyState (&in_forward);
		cl.viewangles[PITCH] += speed * keyboard_speed_pitch.floater * CL_KeyState (&in_back);
	}

	up = CL_KeyState (&in_lookup);
	down = CL_KeyState (&in_lookdown);

	cl.viewangles[PITCH] -= speed * keyboard_speed_pitch.floater * up;
	cl.viewangles[PITCH] += speed * keyboard_speed_pitch.floater * down;

	if (up || down)
		View_StopPitchDrift ();

	// JPG 1.05 - add pq_fullpitch
	if (host_fullpitch.integer)
	{
...
{
	// Baker: research!
	if (cls.signon != SIGNONS) // Baker: JoeQuake didn't ahve this part statement
		return;
	...
	if (in_strafe.state & 1)
	{
		cmd->sidemove += cl_speed_side.floater * CL_KeyState (&in_right);
		cmd->sidemove -= cl_speed_side.floater * CL_KeyState (&in_left);
	}

	cmd->sidemove += cl_speed_side.floater * CL_KeyState (&in_moveright);
	cmd->sidemove -= cl_speed_side.floater * CL_KeyState (&in_moveleft);


#if SUPPORTS_XFLIP
	if(scene_invert_x.integer) cmd->sidemove *= -1;   //Atomizer - GL_XFLIP
#endif


	cmd->upmove += cl_speed_up.floater * CL_KeyState (&in_up);
	cmd->upmove -= cl_speed_up.floater * CL_KeyState (&in_down);

	if (!(in_klook.state & 1))
	{
		cmd->forwardmove += cl_speed_forward.floater * CL_KeyState (&in_forward);
		cmd->forwardmove -= cl_speed_back.floater * CL_KeyState (&in_back);
	}

// adjust for speed key
	if (in_speed.state & 1)
	{
		cmd->forwardmove *= keyboard_speed_move_multiplier.floater;
		cmd->sidemove *= keyboard_speed_move_multiplier.floater;
		cmd->upmove *= keyboard_speed_move_multiplier.floater;
	}
}
...
//	static byte	data[128]; // JPG - replaced with lag_data
	lag = CLAMP (0, cl_net_lag.floater, 400);
...
void CL_BoundViewPitch (float *viewangles)
{
	...
	if (host_fullpitch.integer)
	{
		if (viewangles[PITCH] >  90.0)	viewangles[PITCH] = 90.0;
		if (viewangles[PITCH] < -90.0)	viewangles[PITCH] = -90.0;
	}
	else
	{
		if (viewangles[PITCH] >  80.0)	viewangles[PITCH] = 80.0;
		if (viewangles[PITCH] < -70.0)	viewangles[PITCH] = -70.0;
	}
}
...
void CL_InitInput (void)
{
	...
	Cvar_Registration_Client_Input ();
